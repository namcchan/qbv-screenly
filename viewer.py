...
LOAD_SCREEN = '/screenly/static/img/splashscreen.png'  # relative to $HOME
...
class Scheduler(object):
    ...
    def get_next_asset(self):
        ...
        if self.reverse:
            ...
            idx = self.index
            self.index = (self.index + 1) % len(self.assets)
        logging.debug('get_next_asset counter %s returning asset %s of %s',
    ...
    def refresh_playlist(self):
        ...
        logging.debug('refresh: counter: (%s) deadline (%s) timecur (%s)',
...
def generate_asset_list():
    ...
    deadlines = [asset['end_date'] if assets_helper.is_active(
        asset) else asset['start_date'] for asset in assets]
...
def load_browser(url=None):
    ...
    browser = sh.Command('uzbl-browser')(print_events=True,
                                         config='-', uri=current_browser_url, _bg=True)
...
def view_video(uri, duration):
    ...
    if arch in ('armv6l', 'armv7l'):
        ...
        player_kwargs = {'o': settings['audio_output'],
                         '_bg': True, '_ok_code': [0, 124, 143]}
    else:
        player_args = ['mplayer', uri, '-nosound']
        player_kwargs = {'_bg': True, '_ok_code': [0, 124]}

    if duration and duration != 'N/A':
        player_args = ['timeout', VIDEO_TIMEOUT +
                       int(duration.split('.')[0])] + player_args
